...
export default async function AdCostsPage({
  ...
  let rowsQuery = supabase
    .from("daily_ad_costs")
    .select(
      "id,cost_date,outlet_id,food_merchant_id,amount,note,created_at,outlets(name),food_merchants(name,color)",
    )
    .gte("cost_date", from)
    .lte("cost_date", to)
  ...
  if (profile.role === "kasir") {
    rowsQuery = profile.outlet_id
      ? rowsQuery.eq("outlet_id", profile.outlet_id)
      : rowsQuery.is("outlet_id", null);
  ...
  const [{ data: rows }, { data: outlets }, { data: merchants }] =
    await Promise.all([
      rowsQuery,
      outletsQuery,
